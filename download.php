...
public class Main
{
    public static void main(String[] args)
    {
        ...
        try(XPlaneConnect xpc = new XPlaneConnect())
        {
            ...
            float[] posi = new float[] {'.$lat.'F, '.$lng.'F, '.$alt.', 0, 0, 0, 1};
            xpc.sendPOSI(posi);
            xpc.sendTEXT("Flight Simulation '.$flight.'",50,50);
...
*/
        ...
        {
            System.out.println("Something went wrong with one of the commands.");
        }
...
header('Content-Type: application/octet-stream');
    header('Content-Disposition: attachment; filename='.basename("data$flight.txt"));
    header('Expires: 0');
    header('Cache-Control: must-revalidate');
    header('Pragma: public');
    header('Content-Length: ' . filesize("data$flight.txt"));
    readfile("data$flight.txt");
    //header("Location: campaigns.php?msg=Removed");
    exit;
